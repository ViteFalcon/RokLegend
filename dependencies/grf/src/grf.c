 ...
 * However, when we compile on _WIN32 machine, those types are already defined in windef.h
...
static char *dirname(char *d) {
	if (d == NULL) return ".";
	for(size_t i=strlen(d)-1;i>=0;i--) if ((*(d+i)=='/') || (*(d+i)=='\\')) {
		*(d+i)=0;
		return d;
	}
	return ".";
...
static void prv_grf_free_node(grf_node_handle node) {
	free(node->filename);
...
static void prv_grf_tree_table_free_node(grf_treenode_handle node) {
	if (node->is_dir) {
...
static void prv_grf_reg_tree_node(grf_treenode_handle root, grf_node_handle cur_node) {
	char *fn = cur_node->filename;
	char dirname[4096];
	grf_treenode_handle parent = root;
	grf_treenode_handle new;
	while(1) {
		size_t pos;
		...
		new = (grf_treenode_handle )calloc(1, sizeof(struct grf_treenode));
...
grf_node_handle prv_grf_find_free_space(grf_handle handler, size_t size, grf_node_handle inode) {
    ...
    grf_node_handle cur = handler->first_node;
...
GRFEXPORT bool grf_repack(grf_handle handler, uint8_t repack_type) {
	grf_node_handle node = handler->first_node;
	grf_node_handle prenode;
	uint32_t i=0;
	uint32_t save_pos = 0;
	...
	node = prenode;
	while(node != NULL) {
		grf_node_handle next = node->next;
		i++;
		...
		if (node->pos+node->len_aligned < next->pos) { // found a gap !
            ...
            filemem = (unsigned char *) calloc(1, next->len_aligned + 1024); // 1024 is needed in case of decryption
			...
			if (repack_type >= GRF_REPACK_DECRYPT) {
				// we have at least to decrypt the file, if encrypted.
				if (next->cycle >= 0) decode_des_etc(filemem, next->len_aligned, (next->cycle)==0, next->cycle);
				// clear encryption flags...
				next->cycle = -1;
...
//				count = zlib_buffer_inflate(filenew, next->size, filemem, next->len);
			...
			if (repack_type >= GRF_REPACK_DECRYPT) {
				if (next->cycle >= 0) {
                    ...
                    filemem = (unsigned char *) calloc(1, next->len_aligned + 1024); // 1024 is needed for decryption
					p=0;
					lseek(handler->fd, next->pos + GRF_HEADER_SIZE, SEEK_SET);
					while (p<next->len_aligned) p+=read(handler->fd, filemem+p, next->len_aligned - p);
					decode_des_etc(filemem, next->len_aligned, (next->cycle)==0, next->cycle);
					need_write = true;
					next->cycle = -1;
					next->flags = next->flags & ~(GRF_FLAG_MIXCRYPT | GRF_FLAG_DES);
				}
...
size_t prv_grf_strnlen(const char *str, const size_t maxlen) {
...
GRFEXPORT void grf_create_tree(grf_handle handler) {
	grf_node_handle cur_node;
	uint32_t i=0, j=100;
	if (handler->root != NULL) return;
	// the idea is simple : get to each file and scan them~
	// First, create the root node...
	handler->root = (grf_treenode_handle )calloc(1, sizeof(struct grf_treenode));
...
GRFEXPORT grf_treenode_handle grf_tree_get_root(grf_handle handler) {
	return handler->root;
}

GRFEXPORT grf_treenode_handle *grf_tree_list_node(grf_treenode_handle node) {
	if (!node->is_dir) return NULL;
	return (grf_treenode_handle *)hash_foreach_val(node->subdir);
}

GRFEXPORT bool grf_tree_is_dir(grf_treenode_handle node) {
	return node->is_dir;
}

GRFEXPORT uint32_t grf_tree_dir_count_files(grf_treenode_handle node) {
	return node->subdir->count;
}

GRFEXPORT grf_node_handle grf_tree_get_file(grf_treenode_handle node) {
	return node->ptr;
}

GRFEXPORT const char *grf_tree_get_name(grf_treenode_handle node) {
	return node->name;
}

GRFEXPORT grf_treenode_handle grf_tree_get_parent(grf_treenode_handle node) {
	return node->parent;
}

GRFEXPORT grf_treenode_handle grf_file_get_tree(grf_node_handle handler) {
...
static bool prv_grf_quicksort(struct grf_handler *handler, uint32_t elements) {
  uint32_t beg[PRV_GRF_QUICKSORT_MAX_LEVELS], end[PRV_GRF_QUICKSORT_MAX_LEVELS];
	...
	arr = (grf_node_handle *)hash_foreach_val(handler->fast_table);

  beg[0]=0; end[0]=elements;
  while (i>=0) {
    L=beg[i]; R=end[i]-1;
    if (L<R) {
      piv=arr[L]; if (i==PRV_GRF_QUICKSORT_MAX_LEVELS-1) return false;
      while (L<R) {
...
static bool prv_grf_load(struct grf_handler *handler) {
	...
	uint32_t brokenpos;
	int dlen, result;
	unsigned char *table, *table_comp, *pos, *pos_max;
...
//			if (handler->write_mode) return false; // disallow usage of "write mode" for those files. Repack is needed to write to those files.
			...
			while(pos < pos_max) {
				...
				uint32_t fn_len = (*(uint32_t*)pos)-2; pos+=4+2;
				...
				if (*((entry->filename)+(fn_len-4)) == '.') {
					...
					if (!nocrypt) {
						...
						entry->cycle = 1;
                        for (uint32_t i = 10; (entry->len) >= i; i = i * 10) {
                            entry->cycle++;
                        }
					} else {
		...
		case 0xCACA: // broken-by-repack
            ...
            if ((handler->table_offset + GRF_HEADER_SIZE + (sizeof(uint32_t) * 2) + posinfo[0] + sizeof(uint32_t)) > (uint32_t)grfstat.st_size) {
                return false;
            }
			...
			while(pos < pos_max) {
				size_t av_len = pos_max - pos;
				size_t fn_len = prv_grf_strnlen((char *)pos, av_len);
				struct grf_table_entry_data tmpentry;
				result--;
		...
		case 0x200: // new GRF files
			...
			lseek(handler->fd, handler->table_offset + GRF_HEADER_SIZE, SEEK_SET);
            if (read(handler->fd, (void *)&posinfo, sizeof(posinfo)) != sizeof(posinfo)) {
                return false;
            }
			// posinfo[0] = comp size
			// posinfo[1] = decomp size

            if ((handler->table_offset + GRF_HEADER_SIZE + 8 + posinfo[0]) > (uint32_t)grfstat.st_size) {
                return false;
            }
			table_comp = malloc(posinfo[0]);
			table = malloc(posinfo[1]);
	...
	// overlap check
	grf_node_handle x = handler->first_node;
	uint32_t prev=0;
	while(x != NULL) {
...
GRFEXPORT bool grf_file_rename(grf_node_handle handler, const char *newname) {
	void *rep;
...
GRFEXPORT bool grf_file_delete(grf_node_handle handler) {
	struct grf_handler *parent = handler->parent;
	uint32_t len_aligned = handler->len_aligned;
	grf_node_handle next = handler->next;
...
GRFEXPORT grf_node_handle grf_get_file(grf_handle handler, const char *filename) {
	return hash_lookup(handler->fast_table, filename);
}

GRFEXPORT const char *grf_file_get_filename(grf_node_handle handler) {
	return handler->filename;
}

GRFEXPORT const char *grf_file_get_basename(grf_node_handle handler) {
...
GRFEXPORT uint32_t grf_file_get_size(grf_node_handle handler) {
	return handler->size;
}

GRFEXPORT uint32_t grf_file_get_storage_pos(grf_node_handle handler) {
	return handler->pos;
}

GRFEXPORT uint32_t grf_file_get_storage_flags(grf_node_handle handler) {
	return handler->flags;
}

GRFEXPORT uint32_t grf_file_get_storage_size(grf_node_handle handler) {
	return handler->len_aligned;
}

GRFEXPORT uint32_t grf_file_get_id(grf_node_handle node) {
	return node->id;
}

GRFEXPORT grf_node_handle grf_get_file_by_id(grf_handle handler, uint32_t id) {
	return handler->node_table[id];
}

GRFEXPORT grf_node_handle *grf_get_file_id_list(grf_handle handler) {
	return handler->node_table;
}

GRFEXPORT uint32_t grf_file_get_contents(grf_node_handle fhandler, void *target) {
	void *comp;
...
GRFEXPORT uint32_t grf_file_put_contents_to_fd(grf_node_handle file, int fd) {
	uint32_t size;
	unsigned char *ptr;
	uint32_t p=0;
...
GRFEXPORT bool grf_put_contents_to_file(grf_node_handle file, const char *fn) {
    uint32_t i, p = 0;
	char *name;
	size_t len, size;
	unsigned char *ptr;
...
GRFEXPORT grf_node_handle grf_file_add(grf_handle handler, const char *filename, void *ptr, size_t size) {
...
GRFEXPORT grf_node_handle grf_file_add_fd(grf_handle handler, const char *filename, int fp) {
	void *ptr, *res;
	struct stat s;
...
GRFEXPORT grf_node_handle grf_file_add_path(grf_handle handler, const char *filename, const char *real_filename) {
	int fp;
	void *res;
...
GRFEXPORT grf_node_handle *grf_get_file_list(grf_handle handler) {
	return (grf_node_handle *)hash_foreach_val(handler->fast_table);
}

GRFEXPORT grf_node_handle grf_get_file_first(grf_handle handler) {
	return handler->first_node;
}

GRFEXPORT grf_node_handle grf_get_file_next(grf_node_handle handler) {
	return handler->next;
}

GRFEXPORT grf_node_handle grf_get_file_prev(grf_node_handle handler) {
	return handler->prev;
}
...
#ifdef _WIN32
errno_t ftruncate(int fd, __int64 size)
{
...
static bool prv_grf_write_table(struct grf_handler *handler) {
	...
	if (table_size == 0) {
		table = (unsigned char *)calloc(1, 1);
	} else {
        table = (unsigned char *)malloc(table_size);
		pos=table;
		node = handler->first_node;
		while(node != NULL) {
