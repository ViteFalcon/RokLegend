...
/* (grf_node_handle) grf_file_add(grf_handle, const char *name, void *buffer, size_t size)
 * (grf_node_handle) grf_file_add_fd(grf_handle, const char *name, int fd)
 * (grf_node_handle) grf_file_add_path(grf_handle, const char *name, const char *file)
 * Add a file to the specified GRF file (opened for writing) and return the
 * pointer to the created file.
 */
GRFEXPORT grf_node_handle grf_file_add(grf_handle, const char *, void *, size_t); /* grf.c */
GRFEXPORT grf_node_handle grf_file_add_fd(grf_handle, const char *, int); /* grf.c */
GRFEXPORT grf_node_handle grf_file_add_path(grf_handle, const char *, const char *); /* grf.c */

/* (grf_node_handle) grf_get_file(grf_handle handle, const char *filename)
 * Returns a node handle to the specified file inside the GRF file. If the
 * function fails, NULL is returned.
 */
GRFEXPORT grf_node_handle grf_get_file(grf_handle, const char *); /* grf.c */

/* (const char *) grf_file_get_filename(grf_node_handle)
 * Returns the full filename of a file.
 */
GRFEXPORT const char *grf_file_get_filename(grf_node_handle); /* grf.c */

/* (const char *) grf_file_get_basename(grf_node_handle)
 * Returns only the filename (no path info)
 */
GRFEXPORT const char *grf_file_get_basename(grf_node_handle); /* grf.c */

/* (unsigned int) grf_file_get_size(grf_node_handle)
 * Returns the (real) size of the file.
 */
GRFEXPORT uint32_t grf_file_get_size(grf_node_handle); /* grf.c */

/* (unsigned int) grf_file_get_storage_pos(grf_node_handle)
 * Returns the position of the file in the archive.
 */
GRFEXPORT uint32_t grf_file_get_storage_pos(grf_node_handle); /* grf.c */

/* (unsigned int) grf_file_get_storage_size(grf_node_handle)
 * Returns the real (compressed) size of the file.
 */
GRFEXPORT uint32_t grf_file_get_storage_size(grf_node_handle); /* grf.c */

/* (unsigned int) grf_file_get_storage_flags(grf_node_handle)
 * Test if the first bit is set to know if a node is a file, or not.
 */
GRFEXPORT uint32_t grf_file_get_storage_flags(grf_node_handle); /* grf.c */

/* (unsigned int) grf_file_get_contents(grf_node_handle, void *ptr)
 ...
 */
GRFEXPORT uint32_t grf_file_get_contents(grf_node_handle, void *); /* grf.c */

/* (unsigned int) grf_file_put_contents_to_fd(grf_node_handle, int)
 * Extracts a file to the specified file descriptor. This can be a socket or
 * a regular file, no seeks are used.
 */
GRFEXPORT uint32_t grf_file_put_contents_to_fd(grf_node_handle, int); /* grf.c */

/* (bool) grf_put_contents_to_file(grf_node_handle, const char *filename)
 * Write the contents of the compressed file to the filesystem.
 */
GRFEXPORT bool grf_put_contents_to_file(grf_node_handle, const char *); /* grf.c */

/* (bool) grf_file_rename(grf_node_handle, const char *new_name)
 ...
 */
GRFEXPORT bool grf_file_rename(grf_node_handle, const char *); /* grf.c */

/* (bool) grf_file_delete(grf_node_handle)
 ...
 * will be needed.
 */
GRFEXPORT bool grf_file_delete(grf_node_handle); /* grf.c */
...
/* (grf_treenode_handle) grf_tree_get_root(grf_handle)
 * Returns the root node of the tree.
 */
GRFEXPORT grf_treenode_handle grf_tree_get_root(grf_handle); /* grf.c */

/* (grf_treenode_handle *)grf_tree_list_node(grf_treenode_handle)
 * Returns the list of files/directories found in a specific node.
 * NB: You will have to free() this result after use.
 */
GRFEXPORT grf_treenode_handle *grf_tree_list_node(grf_treenode_handle); /* grf.c */

/* (bool) grf_tree_is_dir(grf_treenode_handle)
 * Return true if this node is a tree. Return false if it's a regular file.
 * There's no other possible type for a node.
 */
GRFEXPORT bool grf_tree_is_dir(grf_treenode_handle); /* grf.c */

/* (const char *) grf_tree_get_name(grf_treenode_handle)
 * Returns the name of a node (no path info will be added)
 */
GRFEXPORT const char *grf_tree_get_name(grf_treenode_handle); /* grf.c */

/* (grf_node_handle) grf_tree_get_file(grf_treenode_handle)
 ...
 */
GRFEXPORT grf_node_handle grf_tree_get_file(grf_treenode_handle); /* grf.c */

/* (grf_treenode_handle) grf_tree_get_parent(grf_treenode_handle)
 * Returns the parent node, or NULL if you call that on the root node.
 */
GRFEXPORT grf_treenode_handle grf_tree_get_parent(grf_treenode_handle); /* grf.c */

/* (grf_treenode_handle) grf_file_get_tree(grf_node_handle)
 * Returns the grf_treenode_handle entry corresponding to a file. Will return NULL
 * if the treenode wasn't created previously.
 */
GRFEXPORT grf_treenode_handle grf_file_get_tree(grf_node_handle); /* grf.c */

/* (unsigned int) grf_tree_dir_count_files(grf_treenode_handle)
 * Returns the number of files contained in a treenode dir.
 */
GRFEXPORT uint32_t grf_tree_dir_count_files(grf_treenode_handle); /* grf.c */

/*****************************************************************************
 ************************** FILE LISTING FUNCTIONS ***************************
 ****************************************************************************/

/* (grf_node_handle*) grf_get_file_list(grf_handle)
 ...
 */
GRFEXPORT grf_node_handle *grf_get_file_list(grf_handle); /* grf.c */

/* (grf_node_handle) grf_get_file_first(grf_handle)
 * Returns the first file's node pointer in the GRF, or NULL if no file were
 * found.
 */
GRFEXPORT grf_node_handle grf_get_file_first(grf_handle); /* grf.c */

/* (grf_node_handle) grf_get_file_next(grf_node_handle)
 * Returns next file, or NULL if file was last file.
 */
GRFEXPORT grf_node_handle grf_get_file_next(grf_node_handle); /* grf.c */

/* (grf_node_handle) grf_get_file_prev(grf_node_handle)
 * Returns previous file, or NULL if file is already the first one.
 */
GRFEXPORT grf_node_handle grf_get_file_prev(grf_node_handle); /* grf.c */

/*****************************************************************************
 ***************************** FILE ID FUNCTIONS *****************************
 ****************************************************************************/

/* (grf_node_handle*) grf_get_file_id_list(grf_handle)
 * Returns the global list of files. The order may change each time you call
 * grf_update_id_list().
 */
GRFEXPORT grf_node_handle *grf_get_file_id_list(grf_handle);
...
/* (unsigned int) grf_file_get_id(grf_node_handle)
 * Returns ID (index position in the list) of a file.
 */
GRFEXPORT uint32_t grf_file_get_id(grf_node_handle); /* grf.c */

/* (grf_node_handle) grf_get_file_by_id(grf_handle, unsigned int id)
 * Returns the file corresponding to the given ID.
 */
GRFEXPORT grf_node_handle grf_get_file_by_id(grf_handle, uint32_t); /* grf.c */
